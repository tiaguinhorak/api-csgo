#!/usr/bin/env bash
# Fetch /api/csgo/stickers/equipped from site (CLUTCH_SITE_RESOLVE_IP when VPS DNS fails).
set -euo pipefail
...
OUT_FILE="${1:-/tmp/clutch-site-stickers.json}"
RESOLVED_SITE_IP=""
...
fetch_stickers_from_url() {
  local base_url="$1"
  local label="$2"

  base_url="${base_url%/}"
  parse_clutch_site_url "${base_url}"
  local url="${base_url}/api/csgo/stickers/equipped"
  ...
  local curl_opts=( -sS --connect-timeout 15 -m 120 --http1.1
  ...
  local http_code
  http_code="$(curl "${curl_opts[@]}" "${url}" 2>/tmp/clutch-site-stickers-curl.err || echo "000")"
  ...
  body_is_valid() {
    ...
    if command -v jq >/dev/null 2>&1; then
      jq -e '.ok == true and (.stickers | type == "array")' "${OUT_FILE}" >/dev/null 2>&1
    else
      grep -q '"stickers"' "${OUT_FILE}"
    fi
  }
  ...
  if [[ "${http_code}" != "200" ]]; then
    if body_is_valid; then
      echo "WARN: curl reported HTTP ${http_code} but payload looks valid — using it." >&2
      return 0
    fi
