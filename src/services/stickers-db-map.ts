/** CSGO Weapon Stickers plugin — weaponstickers1 table (SQLite / MySQL). */
...
export function buildStickerLoadoutSql(
  ...
  for (const entry of entries) {
    ...
    const w0 = (wears[0] ?? 0).toFixed(6);
    const w1 = (wears[1] ?? 0).toFixed(6);
    const w2 = (wears[2] ?? 0).toFixed(6);
    const w3 = (wears[3] ?? 0).toFixed(6);
    const w4 = (wears[4] ?? 0).toFixed(6);
    const w5 = (wears[5] ?? 0).toFixed(6);
