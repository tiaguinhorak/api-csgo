...
bool g_bInitialWeaponsPassDone[MAXPLAYERS + 1];
bool g_bPendingInitialVisualApply[MAXPLAYERS + 1];
...
public Action Timer_ResyncOnlinePlayerStickers(Handle timer) {
    for (int client = 1; client <= MaxClients; client++) {
        ...
        if (ClutchRoutineFullApplyBlocked(client, false)) {
            continue;
        }
        if (
...
void RefreshGlovesNativeFlag() {
    ...
    if (g_bGlovesNativeReady) {
        ...
        if (!g_bLoggedGlovesNativeReadyOnce) {
            g_bLoggedGlovesNativeReadyOnce = true;
...
void ClutchRunCachedVisualPass(int client, bool forceStickers) {
...
void ClutchScheduleInitialVisualApply(int client) {
    ...
    for (int i = 0; i < RESPAWN_VISUAL_PASS_COUNT; i++) {
        ...
        pack.WriteCell(userid);
        pack.WriteCell(1);
        CreateTimer(g_fRespawnVisualDelays[i], Timer_CachedVisualPass, pack, TIMER_FLAG_NO_MAPCHANGE);
    }
}

void ClutchTryFinalizeInitialSync(int client) {
    if (!g_bInitialSyncPending[client]) {
        return;
    }
    if (!g_bInitialWeaponsPassDone[client] || !g_bStickerDbSynced[client]) {
        return;
    }
...
public Action Timer_CachedVisualPass(Handle timer, DataPack pack) {
    pack.Reset();
    int userid = pack.ReadCell();
    bool forceStickers = pack.ReadCell() == 1;
    delete pack;

    int client = GetClientOfUserId(userid);
    if (client > 0 && IsClientInGame(client) && !IsFakeClient(client)) {
        ClutchRunCachedVisualPass(client, forceStickers);
    }
    return Plugin_Stop;
}
...
void ClutchScheduleConnectLoadout(int client) {
    if (client <= 0 || IsFakeClient(client) || !IsClientInGame(client)) {
        return;
    }
    if (g_bConnectApplyScheduled[client]) {
        return;
    }
    ...
    g_bConnectApplyScheduled[client] = true;

    DataPack pack = new DataPack();
    pack.WriteCell(GetClientUserId(client));
    CreateTimer(0.5, Timer_ConnectLoadout, pack, TIMER_FLAG_NO_MAPCHANGE);
}

public Action Timer_ConnectLoadout(Handle timer, DataPack pack) {
    pack.Reset();
    int userid = pack.ReadCell();
    delete pack;
    ...
    if (!IsPlayerAlive(client)) {
        ...
        retry.WriteCell(userid);
        CreateTimer(0.4, Timer_ConnectLoadout, retry, TIMER_FLAG_NO_MAPCHANGE);
        return Plugin_Stop;
    }
...
void ClutchMarkInitialSyncComplete(int client) {
    if (!g_bInitialSyncPending[client]) {
        return;
    }

    g_bInitialSyncPending[client] = false;
    if (g_cvOncePerMatch.BoolValue && !ClutchClientIsSkinAdmin(client)) {
...
void ClutchBeginForcedSync(int client) {
    ...
    g_bInitialWeaponsPassDone[client] = false;
    ...
    fallback.WriteCell(gen);
    CreateTimer(1.2, Timer_ForcedSyncWeaponsFallback, fallback, TIMER_FLAG_NO_MAPCHANGE);
#else
...
#if defined _clutch_gloves_included_
public Action Timer_ForcedSyncWeaponsFallback(Handle timer, DataPack pack) {
    pack.Reset();
    int userid = pack.ReadCell();
    int gen = pack.ReadCell();
    delete pack;
    ...
    if (gen != g_iForcedSyncWeaponGen[client]) {
        return Plugin_Stop;
    }
...
public void OnPlayerSpawn(Event event, const char[] name, bool dontBroadcast) {
    ...
    if (g_bConnectApplyScheduled[client] || g_bInitialSyncPending[client]) {
        return;
    }
...
void ClutchScheduleForcedViewModelStickerPasses(int client) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        int idx = ClutchIndexFromWeaponEntity(client, weapon);
        if (idx < 0 || IsMeleeWeaponKey(g_ClutchWeaponKeys[idx]) || !ClutchWeaponHasStickerCache(client, idx)) {
            continue;
        }
...
void ClutchScheduleViewModelStickerPasses(int client, int weapon, int idx, bool force = false) {
    if (client <= 0 || weapon <= 0 || idx < 0 || !IsValidEntity(weapon)) {
        return;
    }
    ...
    int teamSlot = ClutchStickerTeamSlot(GetClientTeam(client));
    if (!force && ClutchRoutineApplyLocked(client) && ClutchEntityStickersMatchCache(client, weapon, idx, teamSlot)) {
        return;
    }
...
void ClutchSyncViewModelStickersFromWeapon(int client, int weapon, int idx, bool useNative = false) {
    if (client <= 0 || weapon <= 0 || idx < 0 || !IsValidEntity(weapon)) {
        return;
    }

    bool hasCache = ClutchWeaponHasStickerCache(client, idx);
    if (!hasCache && !useNative) {
        return;
    }
    ...
    if (HasEntProp(client, Prop_Data, "m_hViewModel")) {
        for (int slot = 0; slot <= 1; slot++) {
            ...
            if (
                ...
                if (useNative) {
                    ClutchApplyStickersNativePath(client, viewModel, idx, teamSlot, true);
                } else {
    ...
    while ((predicted = FindEntityByClassname(predicted, "predicted_viewmodel")) != -1) {
        ...
        if (useNative) {
            ClutchApplyStickersNativePath(client, predicted, idx, teamSlot, true);
        } else {
...
void ClutchClearAllStickersViaNative(int client, int weapon) {
#if !defined _csgo_weaponstickers_included_
    return;
#else
    if (!ClutchWeaponStickersNativeReady() || client <= 0 || weapon <= 0 || !IsValidEntity(weapon)) {
        return;
    }
...
bool ClutchApplyStickersNativePath(int client, int entity, int idx, int teamSlot, bool force) {
#if !defined _csgo_weaponstickers_included_
    return false;
#else
    if (
    ...
    ClutchClearAllStickersViaNative(client, entity);
...
int ClutchReadStickerIdAtSlot(CEconItemView view, int slot) {
    if (view == view_as<CEconItemView>(0)) {
        return 0;
    }

    int id = view.GetStickerAttributeBySlotIndex(slot, EStickerAttribute_ID, 0);
    if (id != 0) {
        return id;
    }

    int idAttr = 113 + slot * 4;
    CAttributeList staticAttrs = view.AttributeList;
    if (staticAttrs != view_as<CAttributeList>(0)) {
        CEconItemAttribute attr = staticAttrs.GetAttributeByDefIndex(idAttr);
        if (attr != view_as<CEconItemAttribute>(0)) {
            id = attr.Value;
            if (id != 0) {
                return id;
            }
    ...
    CAttributeList demoAttrs = view.NetworkedDynamicAttributesForDemos;
    if (demoAttrs != view_as<CAttributeList>(0)) {
        CEconItemAttribute attr = demoAttrs.GetAttributeByDefIndex(idAttr);
        if (attr != view_as<CEconItemAttribute>(0)) {
            return attr.Value;
        }
    }
...
bool ClutchEntityHasAppliedStickers(int entity, int maxSlots) {
    ...
    for (int s = 0; s < maxSlots; s++) {
        if (ClutchReadStickerIdAtSlot(view, s) != 0) {
            return true;
        }
    }
...
bool ClutchWeaponNeedsStickerSync(int client, int entity, int idx) {
    ...
    int teamSlot = ClutchStickerTeamSlot(GetClientTeam(client));
    return !ClutchEntityStickersMatchCache(client, entity, idx, teamSlot);
}
...
void ClutchScheduleStickerFollowUp(int client, int weapon, int idx, bool force, int teamSlot) {
    ...
    ClutchScheduleViewModelStickerPasses(client, weapon, idx, force);
    if (!ClutchEntityStickersMatchCache(client, weapon, idx, teamSlot)) {
...
#if defined _csgo_weaponstickers_included_
    if (ClutchWeaponStickersNativeReady()) {
        applied = ClutchApplyStickersNativePath(
            client,
            weapon,
            idx,
            teamSlot,
...
#endif
    if (g_bInitialSyncPending[client]) {
        g_bInitialWeaponsPassDone[client] = true;
        ClutchTryFinalizeInitialSync(client);
    }
    return Plugin_Stop;
}
...
public void ClutchGloves_OnClientApplied(int client, int group, int paintkit) {
    ...
    DataPack pack = new DataPack();
    pack.WriteCell(GetClientUserId(client));
    CreateTimer(0.35, Timer_ApplyWeaponsAfterGloves, pack, TIMER_FLAG_NO_MAPCHANGE);
}
#endif
